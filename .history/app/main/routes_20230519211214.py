...
def chat():
    ...
    df = match_query_to_content('what is my sales dashboard')
    xyz = df.loc[0, 'dashboard_title'] 
    ...
    return render_template('chat.html', server_response = xyz,looker_sso_url=sso_url, name=name, room=room, tables=[df.to_html(classes='data')], titles=df.columns.values)
...
def match_query_to_content(query):
    ...
    return return_df
# result = match_query_to_content('what is my sales dashboard')
...
def input_msg():
    ...
    if not query:
        query = 'what is my sales dashboard'
    x = match_query_to_content(query)

    textbox_return = {"input": query, "response": query +
                      'x simple response from server'}
