...
def input_msg():
    ...
    recommended = gcm.CreateLookerEmbedding(sdk=sdk,looker_metadata=looker_metadata, embedding_model=embedding_model, query_question=query).execute()

    looker_metadata['similarity_score'] = recommended
    return_df = looker_metadata[['dashboard_id', 'dash_title', 'embedding_list', 'similarity_score']]

    textbox_return= {"input":msg, "response":msg+' simple response from server'}
