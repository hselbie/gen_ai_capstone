...
class GetDashboardMetadata():
    ...
    def execute(self):
        db_meta = []
        columns = ['dashboard_id', 'title', 'description', 'body_text', 'note_text', 'subtitle_text', 'filterables']
        master_elem_df =pd.DataFrame(columns=columns)
        ...
        for dashboard_id in all_dashboard_id:
            ...
            pd.concat(master_elem_df, elem_df, ignore_index=True)


        elem_meta = [elem for elem in elem_meta if elem]
