        ...
        if dashboard_element_metadata['result_maker']:
            ...
            if listeners:
                for filter in listeners:
                    filterables[filter['name']] = filter.get('dashboard_filter_name')
        return filterables

    def extract_filterables_from_dashboard(self, sdk, dashboard_id: str) -> dict:
        dashboard = sdk.dashboard(dashboard_id=dashboard_id)['dashboard_filter_name']
        return filterables
