...
class GetDashboardMetadata():
    ...
    def execute(self):
        ...
        elem_df= elem_df.groupby(['dashboard_id'])['body_text','note_text','subtitle_text','title','filterables'].apply(lambda x: ','.join(x.astype(str))).reset_index()

        print(elem_df.head(15))
