...
class GetDashboardMetadata():
    ...
    def extract_filterables(self, sdk, dashboard_element_metadata: dict) -> dict:
        filterables = {}
        for key, value in dashboard_element_metadata.items():
            if dashboard_element_metadata['result_maker']:            
                listeners = dashboard_element_metadata['result_maker']['filterables']['listen']
                for filter in listeners:
                    filterables[filter['name']] = filter['dashboard_filter_name']
        return filterables
    ...
    def get_dashboard_element_metadata(self, sdk, dashboard_id: str) -> dict:
        ...
        for dash_element in dash_elements:
            ...
            element_metadata['element_title'] = dash_element['element_title']
            element_metadata['filterables'] = self.extract_filterables(sdk,dashboard_id)
    ...
    def create_metadata_dict(self, dashboard_metadata: dict) -> dict:
        ...
        for key, value in dashboard_metadata.items():
            metadata[key] = value
        return metadata
